...
$(document).ready(function(){
  ...
  // AJAX call to Vimeo gets each video
    ...
    success: function(resp) {
      resp.data.forEach(function(video){
        ...
        if (video.privacy.embed === 'public') {
          $('#embeds').append(`<div class="ui divider"></div><iframe class="myFrame" src="https://player.vimeo.com/video/${video.uri.slice(8, 17)}?autoplay=0&portrait=0" width="100%" height="${$('#embeds').width() * $vidAspect}" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>`);
        }
      })
    }
  })
